  ...
  class NodeQuerier
    ...
    def chef_run(host, options = {})
      ...
      response = if options[:override_recipes]
        ...
        chef_run_response = safe_remote(host) { chef_connection.node.execute_command(host, "chef-client --force-formatter --override-runlist #{cmd_recipe_syntax}", connector: options[:connector]) }

        chef_run_response
      else
        log.info { "Running Chef client on: #{host}" }
        chef_run_response = safe_remote(host) { chef_connection.node.execute_command(host, "chef-client --force-formatter", connector: options[:connector]) }
      end
